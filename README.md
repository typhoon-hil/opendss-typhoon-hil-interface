...
### Generating a New Package File from the Repository

A new Typhoon OpenDSS integration package (`.tpkg`) can be generated by running the [`typhoon_package/build_package.py`](https://github.com/typhoon-hil/opendss-typhoon-hil-interface/blob/1e516e236630d90bdfe571b180594612acff8d65/typhoon_package/build_package.py) script.

#### Prerequisites & Setup (Windows)

1. **Typhoon HIL Control Center:** Ensure Typhoon HIL Control Center is installed. If it is not installed yet, follow the setup instructions at the beginning of this document.
2. **Repository:** Clone this repository to your local machine.
3. **Python Environment:** Ensure Python 3.11.4 or higher is installed.

#### Build Steps

1. Open **Command Prompt** and navigate to the root directory of the cloned repository:
   ```cmd
...
4. Install the required dependencies:
    ...
    ```
5. Run the package generation script:
    ```cmd
    python typhoon_package\build_package.py
    ```
6. Upon successful execution, the generated `.tpkg` file will be located in `typhoon_package\output\`. The file will be named `opendss_XYZ.tpkg`, where `XYZ` corresponds to the version number set in `build_package.py`.
