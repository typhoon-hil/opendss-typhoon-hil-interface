...
class RegControl(GeneralObject):
    ...
    def extra_conversion_steps(self, tse_properties, tse_component):
        ...
        # Set the simulation maximum number of control iterations as needed
        current_maxcontroliter = self.circuit.simulation_parameters.get('maxcontroliter')
        if not current_maxcontroliter or int(tse_properties["control_iters"]) > int(current_maxcontroliter):
            self.circuit.simulation_parameters['maxcontroliter'] = str(tse_properties["control_iters"])
