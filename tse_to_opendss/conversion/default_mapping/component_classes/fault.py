...
class Fault(TwoTerminal):
    ...
    def create_new_format_properties_dict(self, tse_properties, tse_component):
        ...
        new_format_properties = {
            "R": tse_properties['resistance'],
            "phases": str(self.num_phases)
        }
    ...
    def extra_conversion_steps(self, tse_properties, tse_component):
        ...
        self.fault_type = tse_properties["type"]

        if self.fault_type == "A-B-C-GND":
            self.buses.append(f'fbus_{self.name}.0.0.0')
        elif self.fault_type == "A-B-GND":
            self.buses.append(f'fbus_{self.name}.0.0.3')
        elif self.fault_type == "A-C-GND":
            self.buses.append(f'fbus_{self.name}.0.2.0')
        elif self.fault_type == "B-C-GND":
            self.buses.append(f'fbus_{self.name}.1.0.0')
        elif self.fault_type == "A-GND":
            self.buses.append(f'fbus_{self.name}.0.2.3')
        elif self.fault_type == "B-GND":
            self.buses.append(f'fbus_{self.name}.1.0.3')
        elif self.fault_type == "C-GND":
            self.buses.append(f'fbus_{self.name}.1.2.0')
        elif self.fault_type == "A-B":
            self.buses.append(f'fbus_{self.name}.1.1.3')
        elif self.fault_type == "A-C":
            self.buses.append(f'fbus_{self.name}.1.2.1')
        elif self.fault_type == "B-C":
            self.buses.append(f'fbus_{self.name}.1.2.2')
        elif self.fault_type == "A-B-C":
            self.buses.append(f'fbus_{self.name}.1.1.1')
        elif self.fault_type == "None":
            self.buses.append(f'fbus_{self.name}.1.2.3')
