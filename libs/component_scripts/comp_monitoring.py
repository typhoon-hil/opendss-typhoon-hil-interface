...
import subprocess
# import numpy as np
from typhoon.apps.schematic_editor.MyMplCanvas import MatplotlibWidget
        ...
        for line in lines:
            if "new monitor." in line:
                ...
                if not dss_comp_name in self.dss_mon_dict.keys():
                    self.dss_mon_dict[str(dss_comp_name)] = {"name": [element], "mode": [mode], "terminal": [terminal], "monitor name": [str(monitor_name[1])]}
                else:
                    ...
                    (self.dss_mon_dict[str(dss_comp_name)])["monitor name"].append(monitor_name[1])
            ...
            if "new monitor." in line:
                ...
                for wrd in line_elem:
                    if "monitor" in wrd:
                        ...
                        dss_comp_name_temp = monitor_name[1].split("_")
                        dss_comp_name = dss_comp_name_temp[0]
                        if not dss_comp_name in self.dss_comps:
                            self.dss_comps.append(dss_comp_name)

    def update_comp_handle(self, handle):

        self.comp_handle = handle
        print(self.comp_handle)
    ...
    def add_signal_plot_dclck(self, list_item):
        ...
        if current_item1 and current_item2:
            ...
            if not plot_name in self.plot_list_dict.keys():
                ...
                command_line = "export monitors " + str(self.signal_list_dict[str(current_item2.text())])
...
def sim_with_opendss(mdl, mask_handle):
    ...
    dss_file = mdlfile_target_folder.joinpath(dss_folder).joinpath(mdlfile_name + '.dss')
