def v_meas_changed(mdl, mask_handle, v_meas_on):
...
def i_meas_changed(mdl, mask_handle, i_meas_on):
    comp_handle = mdl.get_parent(mask_handle)
    phases = mdl.get_property_value(mdl.prop(mask_handle, "type"))
...
def type_value_changed(mdl, mask_handle, new_type):
    comp_handle = mdl.get_parent(mask_handle)
    ...
    ground_open_circuit(mdl, mask_handle)


def ground_open_circuit(mdl, mask_handle):
    comp_handle = mdl.get_parent(mask_handle)
...
def mask_dynamics(mdl, mask_handle):
