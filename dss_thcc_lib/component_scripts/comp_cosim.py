...
def define_icon(mdl, mask_handle):
    ...
    model_path_prop = mdl.prop(mask_handle, "model_path")
    model_path = mdl.get_property_value(model_path_prop)
    model_path_plib = pathlib.Path(model_path)
    mdl.set_color(mask_handle, "black")
    mdl.disp_component_icon_text(mask_handle, "CoSimulation", relpos_x=0.5, relpos_y=0.3, size=8)
    mdl.disp_component_icon_text(mask_handle, "(Experimental)", relpos_x=0.5, relpos_y=0.4, size=8)

    if model_path_plib.is_file():
        mdl.set_color(mask_handle, "blue")
        mdl.disp_component_icon_text(mask_handle, model_path_plib.stem + ".dss", relpos_x=0.5, relpos_y=0.6, size=8)
    else:
        mdl.set_color(mask_handle, "red")
        mdl.disp_component_icon_text(mask_handle, "Select a model", relpos_x=0.5, relpos_y=0.6, size=8)