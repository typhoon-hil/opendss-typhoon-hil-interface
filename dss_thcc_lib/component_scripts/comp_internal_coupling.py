...
def get_port_const_attributes(phases):
    ...
    port_dict = {"A+": {"pos": comp_positions[0], "term_pos": term_positions[0]},
                 "B+": {"pos": comp_positions[1], "term_pos": term_positions[1]},
                 "C+": {"pos": comp_positions[2], "term_pos": term_positions[2]},
                 "A-": {"pos": comp_positions[3], "term_pos": term_positions[3]},
                 "B-": {"pos": comp_positions[4], "term_pos": term_positions[4]},
                 "C-": {"pos": comp_positions[5], "term_pos": term_positions[5]},
...
def create_coupling_component(mdl, container_handle, coupling_type, phases, force_refactor=False):
    ...
    # Components Vars
    coupling_handle = mdl.get_item("Coupling", parent=comp_handle)
    comp_port_labels = ["A+", "B+", "C+", "A-", "B-", "C-"]
    comp_port_handles = [mdl.get_item(p_name, item_type="port", parent=comp_handle)
                         for p_name in comp_port_labels]
    ground_handles = [mdl.get_item(gname, parent=comp_handle) for gname in ["GND+", "GND-"]]
